...
const mapReportData = (req, data, imageURIs) => {
  ...
  console.log('UserId inside mapReportData:', userId);  

  return {
    userId: userId, 
    companyName: data.body.companyName,
    address: data.body.address,
    BrandTheyUse: data.body.BrandTheyUse,
    notes: data.body.notes,
    images: imageURIs.map((file) => file.imageURI),
  };
};

const createReport = async (req, data, imageURIs) => {
  const reportData = mapReportData(req, data, imageURIs);  
